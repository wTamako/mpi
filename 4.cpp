                ...
                for(k=0;k<n;k++){
                    if(rank==0)
                    {
                        ...
                        for(j=k;j<n;j++)
                            B[k][j]/=tmp;
                        for(j=1;j<size;j++)
                            MPI_Send(&B[k][0],n,MPI_FLOAT,rank+1,k+1,MPI_COMM_WORLD);
                    }
                    else
                        MPI_Recv(&B[k][0],n,MPI_FLOAT,rank-1,k+1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
                    if(r2>=k+1){
                        for(i=max(k+1,r1);i<=r2;i++){
                            ...
                            B[i][k]=0;
                            if (i==k+1&&rank!= 0)
                                MPI_Send(&B[i][0],n,MPI_FLOAT,rank+1,0,MPI_COMM_WORLD);
                        }
                    }
